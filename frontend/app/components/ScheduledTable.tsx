  ...
  if (jobs.length === 0) {
    return (
      <div className="w-full py-16 px-4 flex flex-col items-center justify-center text-center bg-slate-900/40 border border-slate-800/80 rounded-2xl">
        ...
        <h3 className="text-base font-semibold text-slate-200">No Scheduled Emails</h3>
        <p className="text-xs text-slate-400 max-w-sm mt-1">
          You don't have any pending cold emails in queue. Click "Compose Email" above to launch a campaign!
        </p>
      </div>
    );
  }
    ...
    <div className="w-full space-y-4">
            ...
            <TableRow key={job.id}>
              ...
              <TableCell className="text-slate-400 text-xs">
                <div className="flex items-center gap-1.5">
                  ...
                  <span>{job.sender?.name || job.senderId}</span>
                </div>
              </TableCell>
      ...
      {pagination.totalPages > 1 && (
        <div className="flex items-center justify-between px-2 text-xs text-slate-400">
          <div>
            Page {pagination.page} of {pagination.totalPages} ({pagination.total} total scheduled)
          </div>
          <div className="flex items-center gap-2">
            <Button
              variant="outline"
